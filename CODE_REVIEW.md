...
- **Z12** — ~~The working tree has an uncommitted `ZohoBooksClient.xcodeproj/project.pbxproj` change (~231 lines, likely Xcode auto-churn).~~ **Resolved (Aug 17, 2026):** the xcodeproj turned out to be regenerated by Tuist whenever the ZohoAIBookkeeper workspace consumes this package as a local dependency; it's now gitignored/untracked (same policy as `Derived/`), so the tree stays clean.
